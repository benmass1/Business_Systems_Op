...
if all([DB_USER, DB_PASS, DB_HOST, DB_NAME]):
    ...
    DATABASE_URL = os.environ.get(
        'DATABASE_URL',
        'sqlite:///' + os.path.join('/tmp', 'pos_final_secure.db')
    )
...
def index():
    ...
    try:
        ...
        logging.error(e)
        return "KOSA LA DATABASE: Angalia Environment Variables."
...
def sell(id):
    ...
    try:
        ...
        profit = max((p.selling_price - p.buying_price) - disc, 0)
    ...
    if request.method == 'POST':
        ...
        if curr and check_password_hash(shop.admin_password, curr):
            if new and new == conf and len(new) >= 6:
                ...
                flash('Password mpya si sahihi!', 'warning')
        else:
